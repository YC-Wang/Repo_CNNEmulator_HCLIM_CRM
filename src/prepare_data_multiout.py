...
from prepare_data import format_features
# ---

## updated: Feb 3, 2025 by Yi-Chi: 
#  - Change from lat,lon to X,Y
## updated: Mar 8, 2025 by Yi-Chi
#  - add features into the prepare_data.py

    
def create_test_train_split_2ch(config):
    """
    ...
    X['time'] = pd.to_datetime(X.time.dt.strftime("%Y-%m-%d %H:00:00")) # for temperature
    print(X['time'])

#    y = xr.open_dataset(config["y"], chunks = {"time":3000})[config["output_var"][0]]
    y = xr.open_dataset(config["y"], chunks = {"time":3000})[config["output_var"]]
    print(f'output variables: {config["output_var"]}')
    #################
    # if y has NaN (like soil moisture), fill as 0
    y = y.fillna(0)
    ####################################
...
def prepare_training_dataset_2ch(x_train, x_val, x_test,
    ...
    # prepare the rainfall data
        y_train = format_features(y_train).transpose("time","y","x","feature")
        y_test  = format_features(y_test).transpose("time","y","x","feature")
        y_val   = format_features(y_val).transpose("time","y","x","feature")
