...
def train_model(model, x_train, y_train, x_val, y_val, loss, model_weights_name, logdir, epochs, batch_size, optimizer, metrics=['mse']):
    """
...
def predict(model, x_test, y_test, batch_size=32, key='Rain_bc', pred_name='simple_dense', loss='gamma', thres=0.95):
    ...
    #    data = xr.Dataset({key: (["index"], y_test)})
    #else:
    #    data = y_test.to_dataset()
    #data = y_test.to_dataset()
    data = y_test.to_dataset(name=key)
    ...
    if loss == "gamma":
        ...
        rainfall = preds
    data[key].values = rainfall.squeeze()
...
def linear_complex_model(dense_layers=[256], dense_activation='selu', input_shape=(60, 100, 10), dropout=0.6,
    ...
    # plot out model structure
    #from tensorflow.keras.utils import plot_model
    #plot_model(model, to_file='figs_model/model_plot_linear_complex.png', show_shapes=True, show_layer_names=True)
